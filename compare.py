...
outfile = open('./log/' + outfile_name, 'a')
...
for file_name in files1.get_valid_file_list():
    line_cnt = 0
    file_checking1 = open('./result1/'+file_name, 'r')
    file_checking2 = open('./result2/'+file_name, 'r')
    while True:
...
for x in range(len(diff)):
    print('===================================', file=outfile)
    if diff[x][2] == 1:
        print("In result of " + files1.get_name() + ', file name ' + diff[x][0] + ', line ' + str(diff[x][1]) + ':\n' + diff[x][3] + '\n', file=outfile)
        ...
        print('in line ' + str(diff2[x][1]) + ' of ' + files2.get_name() + ', but not in result file of ' + files1.get_name() + '\n', file=outfile)
    else:
